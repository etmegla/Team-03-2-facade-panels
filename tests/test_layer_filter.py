...
from main import (
    _extract_layer_candidates,
    _iter_base_with_inherited_layers,
...
def test_matches_exact_full_path_layer() -> None:
    ...
    assert _match_layer_candidates(
        _extract_layer_candidates(obj),
        "3D-Model::Structure::Floor Plate Curve",
    )
...
def test_matches_leaf_when_object_has_full_path() -> None:
    obj = _obj_with_layer("3D-Model::Structure::Floor Plate Curve")
    assert _match_layer_candidates(_extract_layer_candidates(obj), "Floor Plate Curve")
...
def test_matches_layer_from_properties_or_capital_layer() -> None:
    ...
    assert _match_layer_candidates(_extract_layer_candidates(obj_props), "Floor Plate Curve")
    assert _match_layer_candidates(_extract_layer_candidates(obj_cap), "Floor Plate Curve")


def test_returns_false_for_different_layer() -> None:
    obj = _obj_with_layer("3D-Model::Architecture::Facade")
    assert not _match_layer_candidates(
        _extract_layer_candidates(obj),
        "3D-Model::Structure::Floor Plate Curve",
    )
