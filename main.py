...
class FunctionInputs(AutomateBase):
    ...
    gh_file_path: str = Field(
        ...
        description="Path to the .gh file relative to repo root, e.g. 'assets/test_minimal.gh'",
    )
    ...
    compute_api_key: SecretStr = Field(
        ...
        description="Secret key used to authenticate with Rhino Compute.",
    )
...
def get_or_create_model(client, project_id: str, model_name: str):
    """Return existing model by name, or create it."""
...
def evaluate_gh_with_curves(compute_url: str, api_key: str, gh_path: str, encoded_curves: list) -> dict:
    """
    Call Rhino Compute /grasshopper directly via requests.
    This bypasses compute_rhino3d's r.json() so we can inspect the raw response
    and get a meaningful error if Compute rejects the request.
    """
    # Read and base64-encode the .gh file
    with open(gh_path, "rb") as f:
        algo = base64.b64encode(f.read()).decode("utf-8")
        ...
        "values": [
            {
                ...
                "InnerTree": {
                    "0": [
                        {"type": "System.String", "data": c}
                        for c in encoded_curves
                    ]
                }
            }
        ]
    ...
    headers = {
        ...
        "User-Agent": "compute.rhino3d.py/custom",
    }

    url = compute_url.rstrip("/") + "/grasshopper"
    print(f"  POST {url}  ({len(encoded_curves)} curves, algo size={len(algo)} chars)")

    r = requests.post(url, json=payload, headers=headers, timeout=120)
    print(f"  Compute response: HTTP {r.status_code}  body_len={len(r.text)}")

    if r.status_code != 200:
        raise RuntimeError(
            f"Rhino Compute returned HTTP {r.status_code}: {r.text[:500]}"
        )

    if not r.text.strip():
        raise RuntimeError(
            "Rhino Compute returned HTTP 200 but an empty body. "
            "The .gh file may have failed to load or has no outputs."
        )
...
def automate_function(
    ...
    """Full pipeline: receive → extract Floor Plate curves → GH → send panels."""

    config = _get_config()
    compute_url = config["compute_url"]

    # Resolve API key (function input takes priority over env)
    ...
    if input_key:
        ...
        print("Compute API key source: function input")
    else:
        print(f"Compute API key source: {'environment' if compute_api_key else 'MISSING'}")

    if not compute_api_key:
        automate_context.mark_run_failed(
            "COMPUTE_API_KEY is missing. Set it in Function Inputs."
        )
        return
    ...
    send_to_model(
        ...
        obj=slab_container,
        message=f"Extracted {len(slab_curves)} floor plate curves",
    )
    ...
    for c in slab_curves:
        try:
            if isinstance(c, Line):
                ...
                rc = rhino3dm.LineCurve(start, end)
            elif isinstance(c, Polyline):
                vals = c.value
                pts  = [rhino3dm.Point3d(vals[i], vals[i+1], vals[i+2]) for i in range(0, len(vals), 3)]
                rc   = rhino3dm.PolylineCurve(pts)
            else:  # Curve
                vals = c.points
                pts  = [rhino3dm.Point3d(vals[i], vals[i+1], vals[i+2]) for i in range(0, len(vals), 3)]
                rc   = rhino3dm.PolylineCurve(pts)
            encoded_curves.append(json.dumps(rc.ToNurbsCurve().Encode()))
        except Exception as e:
            print(f"  Skipped curve ({type(c).__name__}): {e}")

    print(f"  Encoded: {len(encoded_curves)} curves")
    if not encoded_curves:
        automate_context.mark_run_failed("Could not encode any curves for Rhino Compute.")
        return

    # ------------------------------------------------------------------
    # STEP 6: Resolve and validate the .gh file path
    # ------------------------------------------------------------------
    gh_path = function_inputs.gh_file_path

    # The container working dir is /home/speckle — resolve relative to it
    if not os.path.isabs(gh_path):
        script_dir = os.path.dirname(os.path.abspath(__file__))
        gh_path_abs = os.path.join(script_dir, gh_path)
    else:
        gh_path_abs = gh_path

    print(f"Grasshopper file: {gh_path!r}  →  resolved: {gh_path_abs!r}")
    print(f"  File exists: {os.path.isfile(gh_path_abs)}")

    # List the assets directory so we can see what's actually there
    assets_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")
    if os.path.isdir(assets_dir):
        print(f"  assets/ contents: {os.listdir(assets_dir)}")
    else:
        print("  assets/ directory does NOT exist in container")

    if not os.path.isfile(gh_path_abs):
        automate_context.mark_run_failed(
            f"Grasshopper file not found at '{gh_path_abs}'. "
            f"Ensure '{gh_path}' is committed to the repo."
        )
        return

    # ------------------------------------------------------------------
    # STEP 7: Run Grasshopper
    # ------------------------------------------------------------------
    print(f"Running Grasshopper: {gh_path_abs}")
    try:
        output = evaluate_gh_with_curves(compute_url, compute_api_key, gh_path_abs, encoded_curves)
    except Exception as e:
        automate_context.mark_run_failed(f"Grasshopper evaluation failed: {e}")
        return

    if output.get("errors"):
        print(f"  GH errors: {output['errors']}")
    if output.get("warnings"):
        print(f"  GH warnings: {output['warnings']}")

    # ------------------------------------------------------------------
    # STEP 8: Decode meshes
    # ------------------------------------------------------------------
    ...
    if not speckle_meshes:
        automate_context.mark_run_failed(
            ...
            "Check that your .gh file outputs Mesh geometry on a named output called 'output'."
        )
        return

    # ------------------------------------------------------------------
    # STEP 9: Send facade panels to model
    # ------------------------------------------------------------------
